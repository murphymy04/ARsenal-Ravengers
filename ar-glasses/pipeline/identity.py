...
class FullIdentity:
    ...
    def _maybe_store_embedding(
        ...
        if (
            frame_count - self._last_embedding_update.get(person_id, 0)
            < EMBEDDING_UPDATE_INTERVAL
        ):
            return False
    ...
    def _update_pending(
        ...
        if best_idx >= 0 and best_score >= PENDING_CLUSTER_SIMILARITY:
            ...
            if n >= MIN_SIGHTINGS_TO_CLUSTER:
                person_id, auto_name = self._db.add_auto_person(
...
def _is_diverse(
    new_emb: FaceEmbedding, existing: list[FaceEmbedding], min_distance: float
) -> bool:
    return all(
